...
def main_app(uid):
    ...
    # ターンCB定義変更
    turn_cb_ip_base = sum(1 for r in records if r.get("last_raiser") and r.get("position") == "IP" and r.get("flop") in ["ベット", "3bet"] and r.get("turn") in ["ベット", "チェック", "レイズ", "3bet", "フォールド", "コール"])
    turn_cb_ip = sum(1 for r in records if r.get("last_raiser") and r.get("position") == "IP" and r.get("flop") in ["ベット", "3bet"] and r.get("turn") in ["ベット", "3bet"])
    turn_cb_oop_base = sum(1 for r in records if r.get("last_raiser") and r.get("position") == "OOP" and r.get("flop") in ["ベット", "3bet"] and r.get("turn") in ["ベット", "チェック", "レイズ", "3bet", "フォールド", "コール"])
    turn_cb_oop = sum(1 for r in records if r.get("last_raiser") and r.get("position") == "OOP" and r.get("flop") in ["ベット", "3bet"] and r.get("turn") in ["ベット", "3bet"])

    # リバーCB定義変更
    river_cb_ip_base = sum(1 for r in records if r.get("last_raiser") and r.get("position") == "IP" and r.get("turn") in ["ベット", "3bet"] and r.get("river") in ["ベット", "チェック", "レイズ", "3bet", "フォールド", "コール"])
    river_cb_ip = sum(1 for r in records if r.get("last_raiser") and r.get("position") == "IP" and r.get("turn") in ["ベット", "3bet"] and r.get("river") in ["ベット", "3bet"])
    river_cb_oop_base = sum(1 for r in records if r.get("last_raiser") and r.get("position") == "OOP" and r.get("turn") in ["ベット", "3bet"] and r.get("river") in ["ベット", "チェック", "レイズ", "3bet", "フォールド", "コール"])
    river_cb_oop = sum(1 for r in records if r.get("last_raiser") and r.get("position") == "OOP" and r.get("turn") in ["ベット", "3bet"] and r.get("river") in ["ベット", "3bet"])

    # Fold to CB定義変更
    fold_to_turn_cb_base = sum(1 for r in records if r.get("flop") == "コール" and r.get("turn") in ["コール", "レイズ", "フォールド"])
    fold_to_turn_cb = sum(1 for r in records if r.get("flop") == "コール" and r.get("turn") == "フォールド")
    fold_to_river_cb_base = sum(1 for r in records if r.get("turn") == "コール" and r.get("river") in ["コール", "レイズ", "フォールド"])
    fold_to_river_cb = sum(1 for r in records if r.get("turn") == "コール" and r.get("river") == "フォールド")
    ...
    if total == 0:
        ...
        st.markdown(f"- Fold to Turn CB%: {fold_to_turn_cb / fold_to_turn_cb_base:.1%} ({fold_to_turn_cb}/{fold_to_turn_cb_base})" if fold_to_turn_cb_base else "- Fold to Turn CB%: なし")
        st.markdown(f"- Fold to River CB%: {fold_to_river_cb / fold_to_river_cb_base:.1%} ({fold_to_river_cb}/{fold_to_river_cb_base})" if fold_to_river_cb_base else "- Fold to River CB%: なし")
